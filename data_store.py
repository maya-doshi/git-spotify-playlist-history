...
def playlist_art_download(path, playlist):
    if playlist['cover'] == "":
        return
    request.urlretrieve(playlist['cover'], os.path.join(path, 'cover.jpg'))
    logging.info('Downloaded ' + playlist['name'] + ' cover')
    return
...
def playlist(id, playlist_data):
    ...
    playlist_art_download(playlist_dir, playlist_data)
